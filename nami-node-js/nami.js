...
class NamiWalletApi {
    ...
    getAccountKey(skey){
        var accountKey = S.PrivateKey.from_normal_bytes(Buffer.from(skey, "hex"))
        // console.log(Buffer.from(accountKey,"hex").toString('hex'))
        var bech32addr = accountKey.to_bech32()
        console.log(bech32addr)
        console.log()
        console.log(this.accountKey.derive(0).derive(0))
        console.log(Buffer.from(this.accountKey.derive(0).derive(0).as_bytes(),'hex').toString('hex'))
    }

    getVkeyForSigning(signingKeyHexCBOR){
        ...
        // vkeysWitnesses.add(vkeyWitness2);
        // return vkeysWitnesses
    }
